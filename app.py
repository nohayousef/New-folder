...
# ✅ Load Model
model_paths = [
    r"C:\Users\NohaA\myenv\finallllllll project\New folder\CUsersNohaAmyenvfinallllllll projectNew foldervoting5_pipeline.pkl", 
...
#     ]  # Add the full list from your dataset
...
# Load location names from dataset
unique_locations = [
    'Adelaide', 'Albany', 'Albury', 'AliceSprings', 'BadgerysCreek',
    'Ballarat', 'Bendigo', 'Brisbane', 'Cairns', 'Canberra'
]  # Replace with full dataset list

# Sidebar location selector
location = st.sidebar.selectbox("🌏 Choose Location", unique_locations)

# 📌 **Prediction Page**
if 'page' in locals() and page == "🔍 Prediction" and 'model' in locals() and model is not None:
    st.title("🔍 Rain Prediction")
    st.sidebar.title("🌍 Enter Weather Details")
    
    # Sidebar inputs
    wind_gust_options = ['W', 'WNW', 'WSW', 'NE', 'NNW', 'N', 'NNE', 'SW', 'ENE', 'SSE', 'S', 'NW', 'SE', 'ESE', 'E', 'SSW']
    wind_gust_dir = st.sidebar.selectbox("💨 Wind Gust Direction", wind_gust_options)
    wind_speed_9am = st.sidebar.slider("🌬️ Wind Speed 9AM (km/h)", 0, 100, 30)
    humidity_9am = st.sidebar.slider("💧 Humidity 9AM (%)", 0, 100, 50)
    temp_9am = st.sidebar.slider("🌡️ Temperature 9AM (°C)", -10, 50, 20)
    temp_3pm = st.sidebar.slider("🌡️ Temperature 3PM (°C)", -10, 50, 25)
    ...
    le_location.fit(unique_locations)
    location_encoded = le_location.transform([location])[0] if location in unique_locations else 0
    
    le_wind = LabelEncoder()
    wind_directions = ['N', 'S', 'E', 'W']
    le_wind.fit(wind_directions)
    wind_gust_encoded = le_wind.transform([wind_gust_dir])[0] if wind_gust_dir in wind_directions else 0
    
    # Prepare input data
    input_data = np.array([
        location_encoded, wind_gust_encoded, wind_speed_9am, humidity_9am, temp_9am, temp_3pm, prev_day_rainfall, rain_today
    ]).reshape(1, -1)
